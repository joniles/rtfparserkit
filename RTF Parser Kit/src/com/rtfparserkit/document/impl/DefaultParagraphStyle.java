...
public class DefaultParagraphStyle extends DefaultCharacterStyle implements ParagraphStyle
{
   ...
   {
      return "ParagraphStyle(" + alignment.name() + ", " + (getFont() != null ? getFont().getName() : "<null>") + "@" + getFontSize() + ", " + (getBold() ? "bold, " : "") + (getItalic() ? "italic, " : "") + (getUnderlined() != UnderlineStyle.NONE ? ("underline: " + getUnderlined().name() + ", ") : "") + "fg: " + getForegroundColor() + ", " + "bg: " + getBackgroundColor() + ")";
   }
}
